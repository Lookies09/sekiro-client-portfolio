...
public class AttackState_NK : EnemyAttackAbleState, IParryableState
{
    public enum AttackPatternState {Normal, Stab}
    private AttackPatternState CurrentPattern;    


    [SerializeField] private AttackPattern_SO[] attackPatterns;

    public Ememy_NK.MeleeAttackVariable meleeAttackVariable;
    ...
    public override void EnterState(int state)
    {
        ...
        int a = Random.Range(0, attackPatterns.Length);
        if (a == 0)
        {
            SetAttackPattern(AttackPatternState.Normal);
        }
        else
        {
            SetAttackPattern(AttackPatternState.Stab);
        }            
    }
    ...
    public void Rebound()
    {
        ...
        controller.TransactionToState((int)EnemyController_NK.STATE.REBOUND);        
    }
